...
That same worker wrapper is also the transport used by `provider_kind = "bulkhead_ssh_peer"`
when one BulkheadLM instance targets another directly over SSH.
...
BulkheadLM locally: `scripts/remote_install.sh`.

## Bootstrap a Clean Client Machine

If machine `A` already has BulkheadLM and machine `B` has only `ssh`, `tar`, and a
POSIX shell, `B` can install a local snapshot of BulkheadLM directly from `A`.

Recommended one-liner on the clean client machine:

```bash
ssh user@remote \
  '/opt/bulkhead-lm/scripts/remote_install.sh --emit-installer --origin user@remote' \
  | sh
```
...
3. installs it into `~/opt/bulkhead-lm`,
...
ssh user@remote \
  '/opt/bulkhead-lm/scripts/remote_install.sh --emit-installer --origin user@remote' \
  | sh -s -- --target "$HOME/apps/bulkhead-lm"
```
...
- the local installer also accepts `BULKHEAD_LM_INSTALL_SSH_BIN` and `BULKHEAD_LM_INSTALL_SSH_ARGS`
  if the callback SSH command needs a non-default binary, port, or identity file
- `BULKHEAD_LM_INSTALL_ARCHIVE_CMD` can override the archive fetch path entirely for custom transports
...
```bash
ssh -t user@remote \
  '/opt/bulkhead-lm/scripts/remote_starter.sh --config /etc/bulkhead-lm/gateway.json'
```
...
```bash
ssh -t user@remote \
  '/opt/bulkhead-lm/scripts/remote_starter.sh --switch 5.2.1-ocamlformat0262'
```
...
```bash
ssh -t user@remote \
  'zsh -lc "/opt/bulkhead-lm/scripts/remote_starter.sh --config /etc/bulkhead-lm/gateway.json"'
```
...
```bash
ssh -T user@remote \
  '/opt/bulkhead-lm/scripts/remote_worker.sh --config /etc/bulkhead-lm/gateway.json --jobs 4'
```
...
```bash
ssh -T user@remote \
  '/opt/bulkhead-lm/scripts/remote_worker.sh --config /etc/bulkhead-lm/gateway.json --api-key sk-bulkhead-lm-prod'
```

## Operational Advice

- Prefer `scripts/remote_starter.sh` for humans and `scripts/remote_worker.sh` for programs.
- Use `scripts/remote_install.sh` when the local client machine does not yet have BulkheadLM at all.
...
```bash
ln -sf /opt/bulkhead-lm/scripts/remote_worker.sh ~/bin/bulkhead-lm-worker
ln -sf /opt/bulkhead-lm/scripts/remote_starter.sh ~/bin/bulkhead-lm-starter
```

Then the SSH entrypoints become:

```bash
ssh -t user@remote bulkhead-lm-starter
ssh -T user@remote bulkhead-lm-worker
```
