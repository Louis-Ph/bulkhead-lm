...
- privacy filtering redacts common contact data, national IDs, payment-card strings, token-shaped secrets, and configured pattern rules from prompt text and structured JSON request fields before upstream dispatch
- threat detection blocks prompt-injection, credential-exfiltration, and tool-abuse signals before provider execution
- output guard blocks private-key material and other configured secret markers before non-streaming responses or materialized streaming responses are returned
...
- audit events and connector session snapshots are privacy-filtered before durable storage when SQLite persistence is enabled
- the administrator control plane exposes a guarded privacy preview endpoint for redaction reporting
...
- provider-native upstream streaming is normalized through a materialized safety pass before client SSE emission, so it is not yet a real-time passthrough
