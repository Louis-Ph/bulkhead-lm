                ...
                (match Viber_connector.find_webhook_config config ~path with
                 ...
                 | None ->
                   (match Wechat_connector.find_webhook_config config ~path with
                    ...
                    | None ->
                      (match Discord_connector.find_webhook_config config ~path with
                       | Some connector -> Some (Discord connector)
                       | None ->
                         Option.map
...
let handle store req body = function
  ...
  | Discord connector -> Discord_connector.handle_webhook store req body connector
