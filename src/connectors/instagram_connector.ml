...
let connector =
  { User_connector_common.channel_name = "Instagram Direct"
  ; provider_id = "instagram-direct"
  ; event_type = "connector.instagram"
  ; session_prefix = "instagram"
  }

let session_limits =
  User_connector_common.session_limits
    ~summary_intro:
      "Compressed memory from earlier in this Instagram Direct conversation. Use it as context, but prefer the recent verbatim turns if they differ."
;;

let default_system_prompt =
  "You are replying through Instagram Direct. Keep answers concise, helpful, and natural for mobile chat. Prefer short paragraphs and direct answers."
;;
...
let help_message =
  String.concat
    ...
    ; "Commands:"
    ; "/reset - clear this Instagram conversation memory"
    ; "/help - show this help"
    ]
;;

let reset_message = "Conversation memory cleared for this Instagram conversation."
let text_only_message = "Send a text message for now. Attachments, stories, and templates are not handled by this connector yet."

let find_webhook_config (config : Config.t) ~path =
  match config.user_connectors.instagram with
...
let send_instagram_message
  ...
  Meta_connector_common.send_text_message
    ...
    ~access_token
    ~endpoint:Meta_connector_common.Me_messages
...
let connector_system_messages
  ...
  =
  let metadata_lines =
    [ Some ("Instagram account id: " ^ message.account_id)
    ; Some ("Instagram scoped user id: " ^ message.sender_id)
    ]
...
let handle_command
  ...
  if reply_text = ""
  then Lwt.return (Ok ())
  else send_instagram_message ~http_post connector_config ~access_token message ~text:reply_text
;;
...
let handle_single_message
  ...
  =
  if
    not (is_allowed_account connector_config message.account_id)
      ...
      (match
       ...
       | Ok () ->
         (match Request_body.parse_json_string raw_body with
          ...
          | Ok json ->
            (match
               Meta_connector_common.parse_inbound_messages ~expected_object:"instagram" json
             with
             | Error err -> Json_response.respond_error err
